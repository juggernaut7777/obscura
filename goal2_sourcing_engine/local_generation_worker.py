...
def discover_products():
    ...
    for subdir in INPUT_DIR.iterdir():
        if subdir.is_dir() and subdir.name not in ("outfit_grid", "_rejected", "__pycache__", "_merged_outfits"):
            ...
            # Why: Avoid N+1 system calls when checking file sizes
            # What: Replace pathlib.glob + stat() with os.scandir to leverage cached OS directory entries
            with os.scandir(subdir) as it:
                for entry in it:
                    name_lower = entry.name.lower()
                    if not entry.is_file() or not name_lower.endswith((".png", ".jpg", ".jpeg", ".webp")):
                        continue
    ...
    # ⚡ Performance optimization
    # Why: Avoid N+1 system calls when stat-ing files in large directories
    # What: Replace pathlib.glob + stat() with os.scandir to leverage cached OS directory entries
