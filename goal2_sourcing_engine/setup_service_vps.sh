...
#
# Usage: sudo bash setup_service_vps.sh
# ============================================
...
echo "🧠 Setting up AI Brain 24/7 service for user: $BRAIN_USER..."

# Create log directory
mkdir -p "$LOG_DIR"
chown "$BRAIN_USER:$BRAIN_USER" "$LOG_DIR"

# Ensure agent_brain.py exists as a symlink to agent_brain_vps.py
if [ ! -f "$BRAIN_DIR/agent_brain.py" ]; then
    echo "🔗 Creating symlink for agent_brain.py -> agent_brain_vps.py..."
    ln -sf "$BRAIN_DIR/agent_brain_vps.py" "$BRAIN_DIR/agent_brain.py"
    chown -h "$BRAIN_USER:$BRAIN_USER" "$BRAIN_DIR/agent_brain.py"
fi
...
python3 -u agent_brain.py --run-hours 6
