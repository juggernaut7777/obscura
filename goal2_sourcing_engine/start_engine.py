    ...
    while True:
        ...
        try:
            ...
            if choice == '1':
                ...
                subprocess.run(["python", "login_socials.py"])
            elif choice == '2':
                print("\n[*] Launching Kling AI Login...")
                subprocess.run(["python", "login_video.py"])
            elif choice == '3':
                print("\n[*] Setting up OBSCURA branding on all platforms...")
                subprocess.run(["python", "profile_setup.py"])
            elif choice == '4':
                print("\n[*] Running Competitor Spy...")
                subprocess.run(["python", "competitor_spy.py"])
            elif choice == '5':
                print("\n[*] Running Factory Hunter...")
                subprocess.run(["python", "factory_hunter.py"])
            elif choice == '6':
                print("\n[!!!] INITIATING 24/7 OBSCURA AUTONOMOUS ENGINE [!!!]")
                time.sleep(2)
                subprocess.run(["python", "agent_brain_local.py"])
            elif choice == '7':
                print("\n[*] Pushing to VPS...")
                subprocess.run(["push_to_vps.bat"])
            elif choice == '8':
                print("\n[*] Running End-to-End Pipeline Test...")
                subprocess.run(["python", "pipeline_test_runner.py", "--self-heal"])
                input("\nPress Enter to return to main menu...")
            elif choice == '9':
                print("\n[*] Running Stock Sync...")
                subprocess.run(["python", "auto_stock_sync.py"])
                input("\nPress Enter to return to main menu...")
            elif choice == '10':
                ...
                if refs:
                    cmd_list.extend(["--refs"])
                    cmd_list.extend(refs.split())
