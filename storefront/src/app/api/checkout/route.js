...
export async function POST(request) {
  try {
    ...
    const commandArgs = ["order_fulfillment.py", "--checkout-file", tempFile];
    
    console.log(`[API CHECKOUT] Executing: python ${commandArgs.join(' ')}`);
    const { stdout, stderr } = await execFileAsync("python", commandArgs, { cwd });
